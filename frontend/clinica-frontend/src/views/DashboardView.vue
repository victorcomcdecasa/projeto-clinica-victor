...
const fazerAgendamento = async () => {
  try {
    ...
    await axios.post('http://127.0.0.1:5000/api/services/agendar', {
...
const buscarAgendamentos = async () => {
  try {
    const token = localStorage.getItem('token');
    const res = await axios.get('http://127.0.0.1:5000/api/services/meus-agendamentos', {
...
onMounted(async () => {
  ...
  try {
    const res = await axios.get(`http://127.0.0.1:5000/api/services/clima/${cidade.value}`);
