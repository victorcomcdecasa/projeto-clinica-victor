...
const buscarCEP = async () => {
  if (form.value.cep.length === 8) {
    ...
    try {
      const res = await axios.get(`https://proviclinicafe.onrender.com/api/services/cep/${form.value.cep}`);
...
const handleRegister = async () => {
  try {
    await axios.post('https://proviclinicafe.onrender.com/api/auth/register', form.value);
